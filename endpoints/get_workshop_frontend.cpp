...
static std::shared_ptr<http_response> handle_endpoint_view(const http_request &request)
{
	...
	if (token.empty() || !user::does_token_exist(token))
		return std::make_shared<file_response>("page/workshop/login.html", 200, "text/html");
	...
	if (args.contains("submission_id"))
	{
		...
		if (!submission_id.empty())
		{
			if (!submission::submission_id_sanitycheck(submission_id))
				return std::make_shared<file_response>("page/workshop/root.html", 200, "text/html");
		...
		auto edit_html_file               = file::read_file("page/workshop/edit.html");
		static const std::string find_str = "const submission_id;";
		edit_html_file                    = edit_html_file.replace(edit_html_file.find(find_str), find_str.size(),
		                                                           "const submission_id = \"" + submission_id + "\";");

		return std::make_shared<string_response>(edit_html_file, 200, "text/html");
	}
